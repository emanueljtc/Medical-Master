<div class="studies index">
	<h2><?php echo __('Studies'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<thead>
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('diagnostic_id'); ?></th>
			<th><?php echo $this->Paginator->sort('person_id'); ?></th>
			<th><?php echo $this->Paginator->sort('name_studie'); ?></th>
			<th><?php echo $this->Paginator->sort('observations'); ?></th>
			<th><?php echo $this->Paginator->sort('created'); ?></th>
			<th><?php echo $this->Paginator->sort('modified'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php foreach ($studies as $study): ?>
	<tr>
		<td><?php echo h($study['Study']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($study['Diagnostic']['id'], array('controller' => 'diagnostics', 'action' => 'view', $study['Diagnostic']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($study['Person']['name'], array('controller' => 'people', 'action' => 'view', $study['Person']['id'])); ?>
		</td>
		<td><?php echo h($study['Study']['name_studie']); ?>&nbsp;</td>
		<td><?php echo h($study['Study']['observations']); ?>&nbsp;</td>
		<td><?php echo h($study['Study']['created']); ?>&nbsp;</td>
		<td><?php echo h($study['Study']['modified']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $study['Study']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $study['Study']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $study['Study']['id']), array(), __('Are you sure you want to delete # %s?', $study['Study']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Study'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Diagnostics'), array('controller' => 'diagnostics', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Diagnostic'), array('controller' => 'diagnostics', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List People'), array('controller' => 'people', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Person'), array('controller' => 'people', 'action' => 'add')); ?> </li>
	</ul>
</div>
